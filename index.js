...
setInterval(() => {
    fetch("https://bot-key-system-2.onrender.com")
        .then(r => console.log("KeepAlive:", r.status))
        .catch(err => console.log("KA Error:", err.message));
}, 5 * 60 * 1000);

// ==============================
// MAIN SERVER + DISCORD BOT
// ==============================
import fs from "fs";
...
function generateKey() {
    return crypto.randomBytes(6).toString("hex"); // 12 char key
}
    ...
    if (cmd === "genkey") {
        ...
        try {
            await message.author.send(
                `🔑 **Your Key**\n**${key}**\n\nValid for **24 hours**.\nUse it inside Roblox Rayfield Script.`
            );
            message.reply("✔ Check your DM! I sent your key.");
        } catch (e) {
            message.reply("⚠ Your DM is blocked! Key here: " + key);
        }
    }
});
